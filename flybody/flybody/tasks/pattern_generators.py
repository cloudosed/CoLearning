...
class WingBeatPatternGenerator():
    ...
    def step_control(self, ctrl_index: float, ctrl_weight = 0.1) -> np.ndarray:
        ...
        # 计算新的 step
        self._step += int(ctrl_index * self._cycle_len * ctrl_weight)
        # 确保 self._step 是循环取值
        self._step = self._step % self._cycle_len
